...
function WearsPageContent() {
  ...
  const handleStoreNavigation = (slug: string) => {
    ...
    setIsStoreDropdownOpen(false);
    router.push(`/shop/wears?store=${slug}`);
              ...
              <div className="col-span-3 md:col-span-1 relative w-full sm:w-48">
                {storeData ? (
                  ...
                  <button
                    ...
                    className={`w-full px-4 py-2.5 rounded-md md:rounded-xl bg-purple-600 hover:bg-purple-700 text-white flex items-center justify-between text-xs md:text-sm font-bold shadow-sm transition-colors ${Boolean(navigatingStoreSlug) ? 'opacity-70 cursor-not-allowed' : ''}`}
                  >
                    <div className="flex items-center gap-2">
                    ...
                    <div className="max-h-60 overflow-y-auto custom-scrollbar">
                      {activeStores.length === 0 ? (
                        ...
                        <button
                          ...
                          className="w-full text-left p-3 hover:bg-purple-50 text-xs font-bold text-gray-800 transition-colors border-b border-gray-50 last:border-0 flex items-center justify-between gap-2"
                        >
                          <span className="truncate">{store.name}</span>
                          <FaStore className="text-purple-500 shrink-0" />
                        </button>
                      ))}
                    </div>
                  </div>
