...
export default function AdminCosmetics() {
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteDoc(doc(db, 'cosmetics', productToDelete));
        ...
        <section className="bg-card p-4 md:p-8 rounded-[var(--radius)] border border-border shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-bold text-pink-700">Vendor</label>
              {isCEO ? (
                <div className="relative">
                  <div
                    onClick={() => setIsVendorDropdownOpen(!isVendorDropdownOpen)}
                    className="w-full p-3 rounded-md border border-pink-200 bg-background flex items-center justify-between cursor-pointer"
                  >
                    ...
                    <FaChevronDown className="text-pink-400 text-xs" />
                  </div>
                  ...
                  {isVendorDropdownOpen && (
                    <div className="absolute top-full left-0 right-0 mt-1 bg-card border border-border rounded-md shadow-xl z-50 max-h-60 overflow-y-auto">
                      ...
                      {admins.filter(a => a.email !== user?.email && (a.assignedRoutes?.includes('/ADMIN/COSMETICS') || a.role === 'CEO')).map(admin => (
                        <div
                          key={admin.id}
                          onClick={() => {
                ...
                <input
                  ...
                  value={user?.email || ''}
                  type="text"
                  className="w-full p-3 rounded-md border border-pink-200 bg-muted text-sm font-semibold text-muted-foreground cursor-not-allowed"
                />
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-pink-700 flex justify-between items-center">
                ...
                <input required value={price} onChange={e => setPrice(formatPriceInput(e.target.value))} type="text" placeholder="e.g. 3,500" className={`w-full p-3 rounded-md border text-sm font-bold outline-none transition-all ${costPrice.trim() !== '' && price.trim() !== '' && (parseFloat(costPrice.replace(/,/g, '')) || 0) >= (parseFloat(price.replace(/,/g, '')) || 0) ? 'border-red-500 focus:border-red-600 ring-2 ring-red-100' : 'border-pink-200 bg-background'}`} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              ...
              <div className="space-y-2">
                ...
                <select required value={isCustomShipping ? 'custom' : size} onChange={e => {
                  ...
                  <option value="custom" className="text-pink-700 font-bold">Customise Shipping Amount</option>
                </select>
                {isCustomShipping && (
                   <div className="mt-2 animate-[slideIn_0.2s_ease]">
                      <label className="text-[0.65rem] font-bold text-pink-700 mb-1 block uppercase">Custom Shipping Amount (₦)</label>
                      <input
                        type="text"
                        ...
                        className="w-full p-2 rounded-md border border-pink-500/50 bg-background text-sm focus:border-pink-600 outline-none transition-all font-bold"
                      />
                   </div>
                )}
              </div>
            ...
            <div className="pt-4 border-t border-border mt-4">
              <div className="max-w-[200px] space-y-2">
                <label className="text-xs md:text-sm font-bold text-pink-700">Quantity In Stock</label>
            ...
            <div className="pt-4 border-t border-border mt-4">
              <label className="flex items-center gap-2 cursor-pointer font-bold text-sm mb-3">
                <input type="checkbox" checked={requiresMinShipping} onChange={e => setRequiresMinShipping(e.target.checked)} className="size-4 accent-pink-600" />
                Require Minimum Quantity for Standalone Shipping?
              </label>
              {requiresMinShipping && (
                <div className="max-w-[200px] space-y-2 animate-in slide-in-from-top-2">
                  <label className="text-xs md:text-sm font-bold text-pink-700">Minimum Quantity Required</label>
                  <input
                    ...
                    type="number"
                    min="1"
                    className="w-full p-2 rounded-md border border-border bg-background text-sm font-bold focus:border-pink-500 outline-none"
                  />
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 pt-4 border-t border-border mt-4">
              <button
                type="submit"
                disabled={loading}
                className={`flex-1 bg-pink-600 text-white py-4 rounded-md font-bold flex items-center justify-center gap-2 text-sm transition-colors ${loading ? 'opacity-70 cursor-not-allowed' : 'hover:bg-pink-700'}`}
              >
